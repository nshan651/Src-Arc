...
static _Bool swapfile_process_running(const ZERO_BL *b0p, const char *swap_fname);
static time_t swapfile_info(char *fname);
static _Bool swapfile_unchanged(char *fname);
static int recov_file_names(char **names, char *path, int prepend_dot) FUNC_ATTR_NONNULL_ALL;
static int ml_append_int(buf_T *buf, linenr_T lnum, char *line, colnr_T len, _Bool newfile, int mark);
