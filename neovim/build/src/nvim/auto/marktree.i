...
static int key_cmp(mtkey_t a, mtkey_t b)
{
  ...
  return (((b.flags) < (a.flags)) - ((a.flags) < (b.flags)));
}
...
{
  map_uint64_t_ptr_t_put(b->id2node, mt_lookup_key(x->key[i]), x);
}
                                                              ...
                                                              rev)
{
  ...
  mtkey_t raw = ((itr)->node->key[(itr)->i]);
  uint64_t other = 0;
  if (mt_paired(raw)) {
...
# 355 "/home/nick/dl/neovim/src/nvim/marktree.c"
  ...
  b->n_keys--;
  map_uint64_t_ptr_t_del(b->id2node, id);
...
void marktree_revise(MarkTree *b, MarkTreeIter *itr, uint8_t decor_level, mtkey_t key)
{
  ...
  ((itr)->node->key[(itr)->i]).flags = (uint16_t)(((itr)->node->key[(itr)->i]).flags & (uint16_t) ~(((uint16_t)(4 - 1)) << 4));
  ((itr)->node->key[(itr)->i]).flags = (uint16_t)(((itr)->node->key[(itr)->i]).flags
                                 | (uint16_t)(decor_level << 4)
                                 | (uint16_t)(key.flags & (((uint16_t)(4 - 1)) << 4)));
  ((itr)->node->key[(itr)->i]).decor_full = key.decor_full;
  ((itr)->node->key[(itr)->i]).hl_id = key.hl_id;
  ((itr)->node->key[(itr)->i]).priority = key.priority;
}
...
# 609 "/home/nick/dl/neovim/src/nvim/marktree.c"
  ...
  mtkey_t k = { .pos = p, .flags = gravity ? (((uint16_t)1) << 14) : 0 };
  if (last && !gravity) {
...
{
  mtkey_t key1 = ((itr1)->node->key[(itr1)->i]);
  mtkey_t key2 = ((itr2)->node->key[(itr2)->i]);
  ((itr1)->node->key[(itr1)->i]) = key2;
  ((itr1)->node->key[(itr1)->i]).pos = key1.pos;
  ((itr2)->node->key[(itr2)->i]) = key1;
  ((itr2)->node->key[(itr2)->i]).pos = key2.pos;
}
...
continue_same_node:
      ...
      if (mt_right(((itr)->node->key[(itr)->i]))) {
        while (!itr_eq(itr, enditr)
               && mt_right(((enditr)->node->key[(enditr)->i]))) {
          marktree_itr_prev(b, enditr);
        }
        if (!mt_right(((enditr)->node->key[(enditr)->i]))) {
          itr_swap(itr, enditr);
