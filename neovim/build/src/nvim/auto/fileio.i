...
static const char *e_auchangedbuf = "E812: Autocommands changed buffer or buffer name";
static const char e_no_matching_autocommands_for_buftype_str_buffer[]
  = "E676: No matching autocommands for buftype=%s buffer";
...
void filemess(buf_T *buf, char *name, char *s, int attr)
{
  ...
  add_quoted_fname(IObuff, (1024 + 1) - 100, buf, name);

  xstrlcat(IObuff, s, (1024 + 1));
...
# 682 "/home/nick/dl/neovim/src/nvim/fileio.c"
  ...
  wasempty = (curbuf->b_ml.ml_flags & 0x01);

  if (!recoverymode && !filtering && !(flags & 0x10) && !silent) {
...
# 2506 "/home/nick/dl/neovim/src/nvim/fileio.c"
  ...
  char *backup_ext = *p_bex == '\000' ? ".bak" : p_bex;
...
# 3005 "/home/nick/dl/neovim/src/nvim/fileio.c"
  ...
  if (converted && wb_flags == 0) {
    ...
    if (write_info.bw_iconv_fd != (iconv_t)-1) {
      ...
      write_info.bw_conv_buflen = (size_t)bufsize * 8;
      write_info.bw_conv_buf = verbose_try_malloc(write_info.bw_conv_buflen);
      if (!write_info.bw_conv_buf) {
        end = 0;
      }
...
# 3436 "/home/nick/dl/neovim/src/nvim/fileio.c"
        ...
        if (os_copy(backup, fname, 0x0002)
            == 0) {
          end = 1;
        }
      } else {
...
# 3599 "/home/nick/dl/neovim/src/nvim/fileio.c"
    ...
    add_quoted_fname(IObuff, (1024 + 1) - 100, buf, fname);
...
# 3936 "/home/nick/dl/neovim/src/nvim/fileio.c"
  ...
  if (ip->bw_iconv_fd != (iconv_t)-1) {
    if (buf_write_convert_with_iconv(ip, bufp, lenp) == 0) {
      return 0;
    }
  }
...
static int buf_write_bytes(struct bw_info *ip)
{
  ...
  if (!(flags & FIO_NOCONVERT)) {
    if (buf_write_convert(ip, &buf, &len) == 0) {
      return 0;
    }
  }
...
static int rename_with_tmp(const char *const from, const char *const to)
{
  ...
  if (strlen(from) >= 4096 - 5) {
    return -1;
  }

  char tempname[4096 + 1];
  strcpy((char *)(tempname), (char *)(from));
  for (int n = 123; n < 99999; n++) {
    char *tail = path_tail(tempname);
    snprintf(tail, (size_t)((4096 + 1) - (tail - tempname - 1)), "%d", n);
...
# 4910 "/home/nick/dl/neovim/src/nvim/fileio.c"
    ...
    } else if (State > (0x1000 | 0x01) || (State & 0x08) || already_warned) {
      if (*mesg2 != '\000') {
      ...
      if (!autocmd_busy) {
        msg_start();
        msg_puts_attr(tbuf, hl_attr_active[(int)(HLF_E)] + 0x1000);
        if (*mesg2 != '\000') {
          msg_puts_attr(mesg2, hl_attr_active[(int)(HLF_W)] + 0x1000);
        }
        msg_clr_eos();
        (void)msg_end();
...
static void vim_mktempdir(void)
{
  ...
  for (size_t i = 0; i < ((sizeof(temp_dirs)/sizeof((temp_dirs)[0])) / ((size_t)(!(sizeof(temp_dirs) % sizeof((temp_dirs)[0]))))); i++) {
    ...
    expand_env((char *)temp_dirs[i], tmp, 256 - 64);
    if (!os_isdir(tmp)) {
      continue;
    }
...
# 5206 "/home/nick/dl/neovim/src/nvim/fileio.c"
    ...
    xstrlcat(tmp, "XXXXXX", sizeof(tmp));
